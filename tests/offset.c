...
static void count_remaining(jwb_ehandle_t track, size_t placed, size_t removed)
{
	jwb_world_t *world = malloc(sizeof(*world));
	struct jwb_world_init alloc_info = JWB_WORLD_INIT_DEFAULT;
	size_t n;
	jwb_ehandle_t e;
	alloc_info.cell_size = 20.;
	alloc_info.flags = JWBF_REMOVE_DISTANT;
	alloc_info.width = 10;
...
int main(void)
{
	count_remaining(1, NUM_ENTS, 0);
	count_remaining(-1, 0, NUM_ENTS);
	return 0;
}
